...
<template>
    <li
        class="group relative overflow-hidden rounded-xl border border-slate-200/70 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-md transition-all p-4 flex items-center gap-3"
    >
        ...
        <transition name="fade-slide">
            <div
            v-if="isRevealed"
            class="mt-1 text-sm font-medium text-indigo-600 bg-indigo-50 border border-indigo-200 rounded px-2 py-1"
            >
        ...
        :disabled="disabled"
        @click="onReveal"
        :title="disabled ? 'Need at least 3 participants' : 'Reveal receiver'"
        >
